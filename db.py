...
#            cursor.execute('create table {0} (ref_designer primary key)'.format(self.tbname))
            ...
            logging.info(
                '{0} do not exists, create new one'.format(self.tbname))
            cursor.execute(
                'create table {0} (Part_Number primary key, value1 char, description char, class char, decal char)'.format(self.tbname))
        cursor.close()
        # 提交事务:
        conn.commit()
        # 关闭Connection:
...
#        conn.close()
    ...
    def writetotable(self, tbname, list_all):
        ...
        try:
            ...
            cursor.execute(
                "insert into {0} values {1}".format(tbname, newlist))
    ...
    def read_part(self, tbname, target_, col, data_):
        ...
        输入
        tbname - str - 表格名称
        target_ - str - 需要读取的属性，如'decal'或者'value'
        col - str - 条件类型，按该类型及值进行查找，如'ref'
        data_ - str - 条件的值，按该值及条件类型进行查找，如'R101'
...
#        logging.info(target_, tbname, col, data_)
        cursor.execute("select {0} from {1} where {2} is '{3}'".format(
            target_, tbname, col, data_))
        b = cursor.fetchall()
#        cursor.close()
#        # 提交事务:
#        conn.commit()
...
#            logging.info('{0}'.format(b))
    ...
    def del_part(self, col, data_):
        conn = sqlite3.connect(self.dbname)
        cursor = conn.cursor()
        cursor.execute("delete from {0} where {1} is '{2}'".format(
            self.tbname, col, data_))
        cursor.close()
        # 提交事务:
        conn.commit()
        # 关闭Connection:
    ...
    def writetotable_list3D(self, tbname, list3D):
        ...
        for a in list3D:
            try:
                ...
                # if e == 'UNIQUE constraint failed: temp.ref':
                logging.warning('{0} 位号重复，无法写入数据库\n{1}\n'.format(a[-1], a))
...
if __name__ == '__main__':
    ...
    device = Device_database(sys.path[0] + '\\test.db')
    device.print_tables()
